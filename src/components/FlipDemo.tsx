  ...
  return (
    <div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-4">
        {items.map((label) => (
          <motion.div
            ...
            className="rounded-md border font-mono text-[12px] px-3 py-3 text-center"
            style={{ borderColor: "var(--line)", background: "var(--bg-raised)", color: "var(--text)" }}
          >
            {label}
          </motion.div>
        ))}
      ...
      <button
        ...
        className="font-mono text-[12px] inline-flex items-center gap-1.5 px-3 py-1.5 rounded border transition-colors"
